--[[
    Teleport Saver
    By Nekomaru Hub
    UI: Obsidian Library (deividcomsono)
    Executor: Delta
]]
...
local function loadRemote(path)
    ...
    warn("[Teleport Saver] Failed to load " .. tostring(path) .. ": " .. tostring(result))
    return nil
end
...
if not Library then
    error("[Teleport Saver] Obsidian Library gagal diload. Cek koneksi/executor HttpGet/loadstring.")
end
...
-- ============================================================
-- Executor file helpers (Delta supports these)
-- ============================================================
...
local CONFIG_FOLDER = "NekomaruHub/TeleportSaver/Checkpoints"
...
local function ensureFolder()
    ...
    if typeof(makefolder) == "function" then
        ...
        if not (typeof(isfolder) == "function" and isfolder(CONFIG_FOLDER)) then
...
-- ============================================================
-- Default checkpoints (dari file yang lu upload)
-- ============================================================
local DEFAULT_CHECKPOINTS = {
...
-- ============================================================
-- Checkpoint UI (dropdown daftar cp)
-- ============================================================
...
local function saveConfig(name)
    if not hasFS then notify("Executor tidak support file", 4); return false end
...
CfgBox:AddButton({
    ...
    Func = function()
        local sel = ConfigDropdown and ConfigDropdown.Value
        if not sel or sel == "" then notify("Pilih config dulu", 2); return end
        local path = CONFIG_FOLDER .. "/" .. sel .. ".json"
        if hasFS and isfile(path) and typeof(delfile) == "function" then
            pcall(delfile, path)
...
-- ============================================================
-- PLAYER TAB (Speed / Jump / Infinite Jump / NoClip)
-- ============================================================
...
-- ============================================================
-- SERVER TAB (Anti AFK, Auto Rejoin, Server Hop)
-- ============================================================
...
task.spawn(function()
    while task.wait(1) do
        local ok = pcall(function()
            ...
            local region = "-"
            pcall(function()
                region = game:GetService("LocalizationService").RobloxLocaleId or "-"
            end)
...
-- Obsidian default settings (dipanggil aman biar beda versi Obsidian/Delta tidak bikin crash)
...
-- Auto load default kalau belum ada config sama sekali
if #listConfigs() == 0 then
    saveConfig("Default_Nekomaru_setup")
